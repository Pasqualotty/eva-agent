...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install eva-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"eva-agent[{extra}]" in spec or f"hermes-agent[{extra}]" in spec
        ]
...
def test_dev_extra_excluded_from_all():
    ...
    assert not any(
        spec in ("eva-agent[dev]", "hermes-agent[dev]")
...
def test_nemo_relay_extra_uses_supported_official_distribution_range():
    ...
    assert not any(
        spec in ("eva-agent[nemo-relay]", "hermes-agent[nemo-relay]")
        for spec in optional_dependencies["all"]
    )
