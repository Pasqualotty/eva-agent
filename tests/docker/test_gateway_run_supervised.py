...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    a poor user experience: the iconic "EVA Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "EVA Gateway
    ...
    assert (
        "⚕" in combined
        or "EVA Gateway Starting" in combined
        or "Hermes Gateway Starting" in combined
    ), (
    ...
    assert (
        "⚕" in file_contents
        or "EVA Gateway Starting" in file_contents
        or "Hermes Gateway Starting" in file_contents
    ), (
