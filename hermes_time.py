"""
Timezone-aware clock for EVA Agent (Hermes fork).

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``EVA_TIMEZONE`` environment variable (``HERMES_TIMEZONE`` compat alias)
  2. ``timezone`` key in agent-home ``config.yaml`` (``~/.eva`` / ``%LOCALAPPDATA%\\eva``)
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — requires 3.11+)
...
def _resolve_timezone_name() -> str:
    ...
    #    EVA_TIMEZONE is canonical; HERMES_TIMEZONE is a one-release alias.
    tz_env = (
        os.getenv("EVA_TIMEZONE", "").strip()
