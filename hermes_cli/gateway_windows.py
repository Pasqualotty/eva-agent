...
_TASK_DESCRIPTION = "EVA Agent Gateway - Messaging Platform Integration"
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"eva {profile_arg} gateway start" if profile_arg else "eva gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: eva gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated EVA gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: eva gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: eva gateway start")
        _print_next_steps()
        return
...
def _print_next_steps() -> None:
    ...
    print()
    print("Next steps:")
    print("  eva gateway status                         # Check status (alias: hermes)")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated EVA gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: eva gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: eva gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `eva gateway status`."
        )
