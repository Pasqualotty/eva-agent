...
class CLICommandsMixin:
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update EVA Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update EVA Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update EVA Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update EVA Agent",
            detail="This will exit the current session and run `eva update`.",
            choices=choices,
        )
        if raw is None:
