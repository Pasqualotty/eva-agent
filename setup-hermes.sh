#!/bin/bash
# ============================================================================
# EVA Agent Setup Script (Hermes fork - local clone)
# ============================================================================
...
# 5. Symlinks the 'eva' (and hermes compat) CLI into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ EVA Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink eva (and hermes compat) into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up eva command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/hermes"
EVA_BIN="$SCRIPT_DIR/venv/bin/eva"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
# Prefer native eva entry point when present; else shim to hermes.
if [ -x "$EVA_BIN" ]; then
    ln -sf "$EVA_BIN" "$COMMAND_LINK_DIR/eva"
elif [ -x "$HERMES_BIN" ]; then
    ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/eva"
fi
if [ -x "$HERMES_BIN" ]; then
    ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/hermes"
fi
echo -e "${GREEN}✓${NC} Symlinked eva → $COMMAND_LINK_DISPLAY_DIR/eva"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# EVA Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
echo "Other commands:"
echo "  eva status        # Check configuration"
if is_termux; then
    echo "  eva gateway       # Run gateway in foreground"
else
    echo "  eva gateway install # Install gateway service (messaging + cron)"
fi
echo "  eva cron list     # View scheduled jobs"
echo "  eva doctor        # Diagnose issues"
echo ""
